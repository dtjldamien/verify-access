  ...
  return new Promise((resolve, reject) => {
    if (baseURL) {
      privateKeyController
        .decryptPrivateKey(
          config.APP_CONFIG.CLIENT_SECURE_CERT,
          config.APP_CONFIG.CLIENT_SECURE_CERT_PASSPHRASE
        )
        .then((res) => {
...
function formURL(
  ...
  let sgverifyURl =
    ...
    state +
    "&timestamp_expiry=" +
    timestampExpiry + // Saturday, 10 October 2020 10:10:10
    "&timestamp_start=" +
    timestampStart + // Thursday, 10 October 2019 10:10:10
    "&v=" +
    version;
...
function assemblingUrlWithSignature(sgverifyURl, signedSgVerifyURl) {
  var sgverifyURlwithSignature =
    sgverifyURl + "&signature=" + signedSgVerifyURl;
